    ...
    switch (status) {
      case 400:
        ...
        if (!data.isSuccess && data.modelErrors) {
          ...
          for (const key in data.modelErrors) {
            if (data.modelErrors[key]) {
...
const Projects = {
  list: (officeId: number) =>
    requests.get<Project[]>(`/Project/list?officeid=${officeId}`),
};
...
const AppUser = {
  ...
  update: (appUser: IAppUser) =>
    requests.put<IAppUser>(`/appuser/${appUser.id}`, appUser),
  delete: (id: string) => requests.del<void>(`/appuser/${id}`),
  lookupUser: (email: string) =>
    requests.get<LookupUser>(`/appuser/lookupusername?email=${email}`),
};
